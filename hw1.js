...
const Student = class Student {
    ...
    constructor(firstName, lastName, gpa, degreeType) {
        ...
        this.gpa = Number(gpa);
        this.degreeType = String(degreeType);
...
const Product = class Product {
    ...
    constructor(description) {
        ...
        this.name = String(desArray[0]);
        this.price = Number(desArray[1]);
        this.availability = String(desArray[2]);
    ...
    static inStock = (products) => {
        return products.filter(products => products.availability.includes('In Stock'));
    }
    ...
    static halfOff = (products) => {
        const copyArray = JSON.parse(JSON.stringify(products)) // had to do this it kept changing the values in products array
        return copyArray.map(function (element) {
    ...
    static printProducts = (products) => {
        function aval(para) {
            ...
            }else if(para=="Out of Stock"){
                return "No"
            }
          }

        //products.forEach(element => console.log("Product: ".concat(element.name).concat(", Cost: ").concat(new Intl.NumberFormat('en-US', {style: 'currency',currency: 'USD',}).format(element.price)).concat(", Availability: ").concat(element.availability)));
        products.forEach(element => console.log("Product: ".concat(element.name).concat(", Cost: ").concat(new Intl.NumberFormat('en-US', {style: 'currency',currency: 'USD',}).format(element.price)).concat(", Availability: ").concat(aval(element.availability))));
       // console.log("Product: Apple, Cost: $1.00, Availability: Yes")
